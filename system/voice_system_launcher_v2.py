...
STT_SCRIPT = "python/new_stt_integrated.py"     # STT service script
...
class VoiceSystemLauncher:
    ...
    def start_process(self, script_name, name, args=None):
        """Start a subprocess"""
        try:
    ...
    def run(self):
        ...
        # Ask user for STT processing method
        print("\nSTT Processing Method:")
        ...
        # Start STT process with pre-selected method
        stt_process = self.start_process(STT_SCRIPT, "STT Service", ["--method", choice])
        if not stt_process:
